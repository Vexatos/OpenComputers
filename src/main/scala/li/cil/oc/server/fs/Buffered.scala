...
object Buffered {
  val fileSaveHandler: WrappedThreadPool = ThreadPoolFactory.createWrappedPool("FileSystem", 1)
}
...
trait Buffered extends OutputStreamFileSystem {
  ...
  override def save(nbt: NBTTagCompound): Unit = {
    super.save(nbt)
    saving = Buffered.fileSaveHandler.withPool(threadPool => threadPool.submit(new Runnable {
