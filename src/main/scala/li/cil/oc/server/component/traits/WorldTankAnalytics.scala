...
import li.cil.oc.util.FluidUtils.GenericBlockWrapper
...
trait WorldTankAnalytics extends WorldAware with SideRestricted {
  ...
  def getTankLevel(context: Context, args: Arguments): Array[AnyRef] = {
    ...
    fluidHandlerAt(position.offset(facing)) match {
      case Some(handler) =>
  ...
  protected def fluidHandlerAt(position: BlockPosition) = FluidUtils.fluidHandlerAt(position) match {
    case Some(handler: GenericBlockWrapper) if handler.currentWrapper.isEmpty => None
